...
public class BrandController {
	...
	public String newBrand(Model model) {
		model.addAttribute("listBrand", service.listAll());
		model.addAttribute("listCategories", serv.listCategoriesUsedInForm());
	...
	public String save(Brand brand,@RequestParam("photoFile")MultipartFile multiPartFile,RedirectAttributes r) throws IOException {
		
		if (!multiPartFile.isEmpty()) {
		String fileName = StringUtils.cleanPath(multiPartFile.getOriginalFilename());
		brand.setLogo(fileName);
		Brand categorySave = service.save(brand);
		String uploadDir = "../brands-image/" + categorySave.getId(); 
		FileUploadUtil.cleanDir(uploadDir);
		FileUploadUtil.saveFile(uploadDir, fileName, multiPartFile);
		}else {
			service.save(brand);
		}
		r.addFlashAttribute("message", "The Brand Save Successfully");
	...
	public String edit(@PathVariable("id") Integer id,RedirectAttributes r,Model model) {
		...
		try {
			model.addAttribute("listBrand", service.listAll());
			model.addAttribute("listCategories", serv.listAll());;
